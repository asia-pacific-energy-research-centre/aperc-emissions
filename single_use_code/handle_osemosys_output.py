#since the output from the 8th edition outlook is in the format that osemosys uses it can be difficult to incroporate into the emisions system. This will just handle verything to do with that so you dont need to.

#MAIN EXPLANATION:
#it will create a emissions factrors set which contains generation factors for the outlook 8th edition but the main point of the file is actually to create a easy to use dataset of emissions and energy use based on the outlook 8th edition data which was originally in the osemosys format.
...
#%%
#improt the data
energy = pd.read_excel('../input_data/00_APEC_reference_results_2022-05-16-153419.xlsx', sheet_name='UseByTechnology')

#emissions factors to calculate emissions:
execfile("../config/config.py")#usae this to load libraries and set variables. Feel free to edit that file as you need

emissions_factors = pd.read_csv('{}/{}'.format(output_data_path, 'outlook_8th_emissions_factors.csv'))

#%%
#analyse the data
#print unique values in the columns
#but we want to exclude any years columns so frst we will melt the data so we have a year and vlaue column
#drop somecols
energy_tall = energy.drop(['Unnamed: 58',	'Unnamed: 59', 'TIMESLICE'], axis=1)

energy_tall = pd.melt(energy_tall, id_vars=['REGION', 'TECHNOLOGY', 'FUEL'], var_name='Year', value_name='Value')
for column in energy_tall.columns:
    if column not in ['REGION', 'Value', 'Year']:
        print(column)
        if column == 'TECHNOLOGY':
            #EXTRACT THE FIRST THREE LETTERS OF THE VALUES IN THE COLUMN AND PRINT THE UNIQUE ONES
            print(energy_tall[column].str[:3].unique())
        else:
            print(energy_tall[column].unique())
...
power_sector_emissions = power_sector.groupby(['Economy', 'Year'])['Emissions', 'Value'].mean().reset_index()
...
energy_tall_emissionms_factors_2 = energy_tall_emissionms_factors.merge(power_sector_emissions[['Economy', 'Year', 'FUEL','emissions_factor']], how='left', on=['Economy', 'Year', 'FUEL'])
...
#%%
#extract the Emissions factor (MT/PJ) column, remove duplicates and save it as outlook_emissions__factors_with_electricity
outlook_emissions_factors_with_electricity = energy_tall_emissionms_factors_2[['FUEL', 'Emissions factor (MT/PJ)']].drop_duplicates()
...
#save in output_data
outlook_emissions_factors_with_electricity.to_csv('../output_data/outlook_emissions_factors_with_electricity.csv', index=False)

# %%
#save the final dataframe as outlook_emissions
energy_tall_emissionms_factors_3.to_csv('data/outlook_emissions_and_energy.csv', index=False)
# %%
