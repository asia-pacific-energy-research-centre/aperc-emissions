...
energy_data_file = '../input_data/00APEC_FUELSUMSREMOVED.xlsx'#CHANGE ME. 
...
for sheet in pd.ExcelFile(energy_data_file).sheet_names:
    ...
    #remove years that are not integers, as they are missing data
    APEC_data = APEC_data[APEC_data['Year'].apply(lambda x: isinstance(x, int))]
...
apec_factors_2 = dummy_df.merge(emissions_factors, how='cross')#cross joins are amazing
